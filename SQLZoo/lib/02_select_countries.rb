...
def high_population_gdps
  ...
  execute(<<-SQL)
  SELECT countries.name, countries.gdp/countries.population
  FROM countries
  WHERE countries.population > 200000000
  SQL
end
...
def population_in_millions
  ...
  # millions.
  execute(<<-SQL)
  SELECT countries.name, countries.population/1000000
  FROM countries
...
def name_and_population
  ...
  FROM countries
  WHERE countries.name = 'France' OR countries.name = 'Germany' OR countries.name = 'Italy' 
  SQL
end
